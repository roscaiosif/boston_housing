...
def ModelLearning(X, y):
    ...
    for k, depth in enumerate([1,3,6,10]):
        ...
        #print 'd: {} \n trainscores: {}'.format(depth,train_scores)
        #print type(train_mean), train_mean.shape
        print 'd: {:d}\n train:{} \n test:{}'.format(depth,train_mean,test_mean)
...
def PredictTrials(X, y, fitter, data):
    ...
    for k in range(10):
        ...
        # Make a prediction
        pred = fitter.predict(data)
        prices.append(pred)
        
        # Result
        print "Trial {}".format(k+1)
        for i in range(data.shape[0]):
            print "client {}: ${:,.2f}".format(i+1, float(pred[i]))

    # Display price range
    prices = np.array(prices)
    for i in range(data.shape[0]):
        print "\nClient {:d} Range in prices: ${:,.2f}".format(i+1,max(prices[:,i]) - min(prices[:,i]))